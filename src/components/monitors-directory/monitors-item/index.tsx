...
import ARROW_SVG from 'svgs/ui/arrow-down.svg';
...
const MonitorsItem = ({ data }: { data: MonitorTypes }) => {
  ...
  return (
    <div className="first:b-t-secondary-50 space-y-3 border-b border-b-secondary-50 py-4 first:border-t">
        ...
        <div className="flex space-x-3">
          <AnimatePresence>
            {/* <motion.div whileHover="hover" variants={variants.icon}>
              <Icon icon={ARROW_SVG} className="inline-block h-3 w-3 -rotate-90" />
            </motion.div> */}
      ...
      <button type="button" className="flex items-center space-x-3" onClick={handleVisibility}>
        ...
        />
        <p>
          Show <span className="rounded-sm bg-secondary-50 px-2 py-0.5">{geostoriesSentence}</span>
        </p>
      </button>
      <AnimatePresence>
        <motion.ol
